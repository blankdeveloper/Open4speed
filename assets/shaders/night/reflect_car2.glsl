...
void main()
{
  ...
  gl_FragColor.rgb = (diffuse.a < 0.75) ? 6.0 * diffuse.rgb * ((u_brake > 0.5) ? 1.0 : 0.2) : 0.1 * diffuse.rgb;
  gl_FragColor.a = 1.0;
  
  vec3 N = v_Normal - 0.25 + 0.5 * diffuse.rgb;
  
  //reflect
  vec4 r = texture2D(EnvMap1, vec2(0.5 + N.x * 0.5, 1.5 - gl_FragCoord.z * 1.0 + N.z * 0.5)) * 0.5 * clamp(0.0, (v_Vertex2.y - 0.4), 1.0);
