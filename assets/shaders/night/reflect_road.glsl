...
void main()
{
  ...
  //diffuse light
  gl_FragColor.rgb += 10.0 * max(max(dot(N, L), 0.0) * diffuse.rgb - length(lightDir) * 0.001, 0.0) * (eff - u_light_cut);
  //specular light
  vec3 R = normalize(-reflect(L, N));
  vec3 E = normalize(-v_Vertex);
  gl_FragColor.rgb += diffuse.a * diffuse.rgb * pow(abs(dot(R, E) * dot(N, diffuse.rgb)), 2.0) * 0.1;
  //gl_FragColor.rgb *= pow(eff, u_light_spot_eff);
  gl_FragColor.a = 1.0;
}
END