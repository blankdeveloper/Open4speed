...
Míchání textur se v OpenGL provádí pomocí blendingu. Tato technika míchá cílový obraz (ten je již vykreslený ve FBO) a zdrojový obraz, tedy texturu namapovanou na nějaký model.
...
\end{array}$
\end{center}
\caption{Příklad neprůhledného a poloprůhledného materiálu}
\end{figure}
...
\begin{center}
$S_{ABC} = 0.5 \cdot (\vec{AB} \times \vec{AC})$\\
$S_{ABP} = 0.5 \cdot (\vec{AB} \times \vec{AP})$\\
$S_{APC} = 0.5 \cdot (\vec{AP} \times \vec{AC})$\\
$S_{PBC} = 0.5 \cdot (\vec{PB} \times \vec{PC})$
\end{center}
